async function cargarServicio(ev) {
  ...
  var action = "../../back/servicio/listar.php";
    ...
    if (response.ok) {
	  ...
	  if ( respuesta.length > 0 ) {
		  ...
		  for (item of respuesta) {			  
			  filas += "<option ";
			  filas += "value='"+item.id_servicio+"'>";
			  filas += item.nombre_servicio;
			  filas += "</option>";
		  } // End for
	  } else {
  ...
  cargarServicio.enviando = false;
}

document.addEventListener("DOMContentLoaded", function() {
  window.addEventListener("load", cargarServicio);
});