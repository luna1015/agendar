async function cargarEsteticista(ev) {
  ...
  var action = "../../back/esteticista/listar.php";
    ...
    if (response.ok) {
	  ...
	  if ( respuesta.length > 0 ) {
		  ...
		  for (item of respuesta) {			  
			  filas += "<option ";
			  filas += "value='"+item.id_esteticista+"'>";
			  filas += item.nombre_esteticista;
			  filas += " ";
			  filas += item.apellidos_esteticista;
			  filas += "</option>";
		  } // End for
	  } else {
  ...
  cargarEsteticista.enviando = false;
}

document.addEventListener("DOMContentLoaded", function() {
  window.addEventListener("load", cargarEsteticista);
});