async function cargarFormulario(ev) {
  ...
  var datos = { 
				id_esteticista: form.elements['id'].value
			};
    ...
    if (response.ok) {
	  ...
	  if ( respuesta.length > 0 ) {
		  for (item of respuesta) {
			  form.elements['documento'].value = item.numero_documento_esteticista;
			  form.elements['tipoDocumento'].value = item.tipo_documento_esteticista;
			  form.elements['nombre'].value = item.nombre_esteticista;
			  form.elements['apellidos'].value = item.apellidos_esteticista;
			  form.elements['fechaNacimiento'].value = item.fecha_nacimiento_esteticista;
			  form.elements['direccion'].value = item.direccion_esteticista;
			  form.elements['celular'].value = item.celular_esteticista;
			  form.elements['email'].value = item.email_esteticista;
			  form.elements['activo'].value = item.activo_esteticista;
		  } // End for
