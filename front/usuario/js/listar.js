    ...
    if (response.ok) {
	  ...
	  if ( respuesta.length > 0 ) {
		  for (item of respuesta) {
			  ...
			  filas += "<td>"+item.nombre_usuario+"</td>";
			  filas += "<td>"+item.apellidos_usuario+"</td>";
			  filas += "<td>"+item.nombre_rol+"</td>";
			  filas += "<td align='center'>";
			  filas += "<a href='detalleUsuario.php?id="+item.id_usuario+"'><img src='../imagen/detalle.png'></a>";
			  filas += "<a href='editarUsuario.php?id="+item.id_usuario+"'><img src='../imagen/editar.png'></a>";
			  filas += "<a href='' onclick='return eliminarRegistro("+item.id_usuario+")'>";
...
async function enviarFormulario(ev) {
  ...
  //var datos = new FormData(form);
  var datos = { 
				id_rol: form.elements['id_rol'].value,
